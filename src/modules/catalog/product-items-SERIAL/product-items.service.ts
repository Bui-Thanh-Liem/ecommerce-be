...
export class ProductItemsService {
  ...
  async findAll(query: ProductItemQueryDto): Promise<IMetadata<ProductItemEntity>> {
    ...
    //
    const queryBuilder = this.productItemRepo
      .createQueryBuilder('productItem')

      // Join các quan hệ
      .leftJoinAndSelect('productItem.inventory', 'inventory')
      .leftJoinAndSelect('productItem.productVariant', 'productVariant')
      ...
      .select([
        ...
        'productItem.locationInWarehouse',
        'productItem.status',
        'productItem.warrantyActivatedAt',
      ...
      .skip(skip)
      .take(take)
      .orderBy('productItem.createdAt', 'DESC'); // Nên có orderBy khi phân trang
