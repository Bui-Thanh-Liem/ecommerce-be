...
export class MenuService {
  ...
  async create(dto: CreateMenuDto) {
    ...
    //
    const [existMenu, existCategorySlug, existMenuInCategory] = await Promise.all([
      this.menuRepository.exists({ where: { name } }),
      this.categoryService.findSlugById(categoryId),
      this.menuRepository.findOne({ where: { category: { id: categoryId } } }),
    ]);
    ...
    if (!existCategorySlug) {
      throw new NotFoundException('Category not found');
    }
    if (existMenuInCategory) {
      throw new NotFoundException('A menu for this category already exists');
    }

    //
    const menu = this.menuRepository.create({
      ...dto,
      categorySlug: existCategorySlug,
  ...
  async findAll(query: MenuQueryDto): Promise<IMetadata<MenuEntity>> {
    ...
    //
    if (filters?.name) {
      queryBuilder.andWhere('unaccent(menu.name) ILIKE unaccent(:name)', { name: `%${filters.name}%` });
    }

    // Phân trang và sắp xếp
    queryBuilder.orderBy('menu.createdAt', 'DESC').skip(skip).take(take);
  ...
  async update(id: string, dto: UpdateMenuDto) {
    ...
    //
    const [exist, existCate, existMenuInCategory] = await Promise.all([
      name ? this.menuRepository.exists({ where: { name, id: Not(id) } }) : null,
      categoryId ? this.categoryService.exists([categoryId]) : null,
      categoryId ? this.menuRepository.findOne({ where: { category: { id: categoryId }, id: Not(id) } }) : null,
    ]);

    //
    if (name && exist) throw new NotFoundException('A menu with the same name already exists');
    if (categoryId && !existCate) throw new NotFoundException('Not found category');
    if (categoryId && existMenuInCategory) throw new NotFoundException('A menu for this category already exists');
